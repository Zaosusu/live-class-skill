---
name: live-class-copilot
description: "直播课/线上课程/会议「代听」助手：自动打开直播网址→持续录制系统声音→本地转写成文字→结束后生成结构化总结笔记。当用户说「帮我听/代听这场直播」「把 B站/抖音/小红书直播课录下来转成文字」「没时间上课，结束后给我一份笔记/总结」「替我去听会，结束时给我要点」，或给出一个直播/课程/会议 URL 要求收音转写并总结时使用。适用于任何能在浏览器出声的线上直播课（B站/抖音/小红书/视频号/腾讯会议等）。"
agent_created: true
---

# live-class-copilot — 直播课/会议「代听」助手
...
- **转写引擎 sherpa-onnx 本地离线**（中文 zipformer-ctc int8 模型，约 370MB，
  缓存于 `~/.cache/live-class-copilot/models/`），不联网、不花钱、速度快（远超实时）。
...
1. 用浏览器自动化打开直播 URL；需登录则引导用户扫码（登录态会保留）；
2. 确认直播在播放、**系统/页面音量非 0**、标签页未被静音
   （ScreenCaptureKit 取的是系统混音，静音时录不到——用户可正常外放）；
3. 等约 25 秒让第一个分块落地，检查是否转出文字：
   ```bash
   tail -1 "$SESSION/transcripts/segments.jsonl"
   ```
   - 有文字 → 链路通了，向用户汇报「已开始代听」，进入监控。
   - 为空/(静音) → 排查：① 系统音量是否为 0；② 页面是否真在播放
     （可截图确认）；③ 平台是否要登录/权限。修复后重试。
