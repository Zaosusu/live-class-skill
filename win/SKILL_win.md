...
description: "社媒直播课 / 线上课程「代听」助手（Windows 版）：用户没时间跟社媒平台上的直播课/线上课程（B站/抖音/小红书/视频号/快手/千聊/小鹅通等），自行打开直播网页出声 → 用 Windows 系统自带 WASAPI loopback 内录系统声音（零第三方、零浏览器自动化）→ sherpa-onnx 本地离线转写 → 结束后生成结构化总结笔记。当用户说「帮我听/代听这场直播课」「把 B站/抖音/小红书直播课录下来转成文字」「没时间上课/看直播，结束后给我一份笔记/总结」，且运行环境是 Windows 时使用。会议类（腾讯会议/飞书会议/Zoom）已自带 AI 纪要总结，非本技能目标场景。⚠️ 本技能绝不使用任何浏览器自动化（防封号）：只负责「监听系统声音」，打开直播页由用户自己完成。"
agent_created: true
---

# live-class-skill（Windows 版）— 社媒直播课/线上课程「代听」助手
...
> 仅当某场会议确无 AI 纪要又必须代听时，本技能技术上同样能内录（见 `win/references/windows-audio-setup.md`），但非主场景。

> **平台入口**：本文件是 Windows 版完整流程。mac 版见仓库根 `SKILL.md`。
> 两版共享同一套离线 ASR 引擎与模型（通用层在仓库根 `scripts/`：`transcribe.py`、`common.py`）。

## ⚠️ 铁律：零浏览器自动化（防封号）

本技能**只监听系统正在播放的声音**，**绝不驱动、注入、控制、读取任何浏览器/App**。
打开直播网页这一步由**用户自己**在系统默认浏览器里完成，或在桌面 App（如小鹅通/千聊客户端）里打开。
...
- **内录零安装、零第三方**：直接用 Windows 系统自带的 **WASAPI loopback**（Core Audio 的回环采集）
  抓"默认输出设备正在播放的声音"（OBS 等录"扬声器输出"的软件同款官方接口）。
